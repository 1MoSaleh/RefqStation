<article class="card my-3 mx-3  col-9 col-sm-5 col-md-4 col-lg-3 col-xl-3" style="border-bottom: 3px solid #00BDAF; max-width: fit-content; min-width: fit-content;">
    <div class = 'card-body' style=" min-width: 250px">
        <a href="{{route('adoption.show', $card->id)}}" class = 'card-body' style="color:black ; text-decoration:none; min-width: 250px">
        ...
        <div class="">
            ...
            <label> <small class="text-muted mr-1"> <i class="fas fa-question mainColor mr-1"></i>{{__("فئة الحيوان :")}} </small>{{__($card->pet->type)}}</label> <br>
            <label> <small class="text-muted mr-1"> <i class="far fa-calendar-alt mainColor mr-1"></i>{{__("الفئة العمرية :")}}</small>{{__($card->pet->age)}}</label> <br>
            <label> <small class="text-muted mr-1"><i class="fas fa-venus-mars mainColor mr-1"></i>{{__("الجنس :")}}</small>{{__($card->pet->gender)}} </label> <br>
            <label> <small class="text-muted mr-1"> <i class="fas fa-file-medical-alt mainColor mr-1"></i>{{__("التعقيم :")}}</small>{{__($card->pet->castration)}}</label> <br>
            <label> <small class="text-muted mr-1"> <i class="fas fa-book-medical mainColor mr-1"></i>{{__("التطعيمات :")}}</small>{{__($card->pet->vaccinated)}}</label> <br>
            ...
            <div>
                <a href="{{route('adoption.show', $card->id)}}" class ='' style="color:black ; text-decoration:none; min-width: 250px">
